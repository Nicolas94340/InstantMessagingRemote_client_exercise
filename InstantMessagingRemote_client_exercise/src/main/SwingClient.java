...
public class SwingClient {
  ...
  class newPublisherHandler implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      String element = argument_TextField.getText();
      Topic t = new Topic(element);
      if(topicManager.topics().contains(t)==false){
            publisher_TextArea.append(t.name + "\r\n");
      }
      publisherTopic = new Topic(element);
  ...
  class newSubscriberHandler implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      String element = argument_TextField.getText();
      Topic t = new Topic(element);
      Subscription_check sc = topicManager.subscribe(t, subscriberImpl);
      if(sc.result == Subscription_check.Result.OKAY)
      {
          my_subscriptions.put(t, subscriberImpl);
          messages_TextArea.append("Subscription OK" + "\n");
      }
      else 
      {
          messages_TextArea.append("Subscription not OK" + "\n");
      }
      for (Map.Entry mapentry : my_subscriptions.entrySet()) {
           my_subscriptions_TextArea.append(((Topic)mapentry.getKey()).name +"\r\n");;
        }
  ...
  class UnsubscribeHandler implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      String element = argument_TextField.getText();
      Topic t = new Topic(element);
      subscriberImpl = (SubscriberImpl) my_subscriptions.get(t);
      if(subscriberImpl != null)
      {
          subscriberImpl.onClose(new Subscription_close(t,Subscription_close.Cause.SUBSCRIBER));
      }
      topicManager.unsubscribe(t, subscriberImpl);
  ...
  class postEventHandler implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      publisher.publish(new Message(publisherTopic, element));
      messages_TextArea.append( element + "\r\n");
